...
class PortefeuilleTest {
    ...
    void testAddAndRemoveActions() {
        ...
        //Our test will include the insertion and removal of two actions in one Portefeuille
        ...
        //We add the first action, then check if the action is correctly added by comparing it to our predefined String structure
        portefeuille.acheter(action1, 10);
        assertEquals("{Action1=10}", portefeuille.toString());

        
        //We add the second action, then check if both actions are represented in the correct way in order to verify the addition
        portefeuille.acheter(action2, 20);
        assertEquals("{Action1=10, Action2=20}", portefeuille.toString());

        
        //We add more to our first action and expect to see the addition on our Portefeuille.
        portefeuille.acheter(action1, 5);
        assertEquals("{Action1=15, Action2=20}", portefeuille.toString());

        
        //We remove from the first action, we expect to see the decrease on our action quantity
        portefeuille.vendre(action1, 10);
        assertEquals("{Action1=5, Action2=20}", portefeuille.toString());

        
        //Here we sell all the quantity of our first action, we expect to see only our second action and its value
        portefeuille.vendre(action1, 5);
        assertEquals("{Action2=20}", portefeuille.toString());

        
        //Here we sell again from the first action, we expect to have no effects on our Portefeuille since all of the action1 is sold
        portefeuille.vendre(action1, 5);
        assertEquals("{Action2=20}", portefeuille.toString());

        //Here we sell the rest of the second action to verify the Portefeuille will be empty
